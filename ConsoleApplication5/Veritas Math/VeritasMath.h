...
inline bool __cdecl VMScalarNearEqual
(
	float S1,
	float S2,
	float Epsilon
...
inline VMMATRIX __vectorcall VMLoadFloat3x3
(
	const VMFLOAT3X3* pSource
)
{
	assert(pSource);
...
{
	assert(!VMScalarNearEqual(ViewWidth, 0.0f, 0.00001f));
	assert(!VMScalarNearEqual(ViewHeight, 0.0f, 0.00001f));
	assert(!VMScalarNearEqual(FarZ, NearZ, 0.00001f));
...
inline VMMATRIX __vectorcall VMMatrixTranslationFromVector
(
	FVMVECTOR Offset
)
{
	...
	M.r[1] = g_XMIdentityR1.v;
	M.r[2] = g_XMIdentityR2.v;
	M.r[3] = VMVectorSelect(g_XMIdentityR3.v, Offset, g_XMSelect1110.v);
	return M;
}
#pragma endregion
